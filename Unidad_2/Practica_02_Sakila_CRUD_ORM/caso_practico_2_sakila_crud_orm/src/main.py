...
def inventory_menu(service):
    ...
    if option == "1":
        ...
        for film in service.films.list(20):
            print(f"{film.film_id}: {film.title}")
        print("\nTiendas disponibles:")
        for store in service.stores.list(10):
            print_entity(store)
...
def store_menu(service):
    print("\nTiendas")
    print("1. Buscar tienda por ID")
    print("2. Listar tiendas")
    option = input("Opcion: ").strip()

    if option == "1":
        print_entity(service.stores.read(read_int("store_id: ", minimum=1)))
    elif option == "2":
        for store in service.stores.list(read_int("Limite: ", default=10, minimum=1)):
            print_entity(store)
    else:
    ...
    try:
        while True:
            try:
                ...
                print("4. Inventario")
                print("5. Tiendas")
                print("6. Metricas descriptivas de peliculas")
                print("7. Historial de operaciones")
                print("0. Salir")
                option = input("Opcion: ").strip()
