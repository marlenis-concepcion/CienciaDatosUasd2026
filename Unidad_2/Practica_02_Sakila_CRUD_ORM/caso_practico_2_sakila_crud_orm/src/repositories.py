from src.models import City, Country, Film, Inventory, ModelCollection, Store
...
# /****
# Repositorio de consulta para las tiendas existentes en Sakila.
# ****/
class StoreRepository(BaseRepository):
    def __init__(self, db, history=None, cache=None):
        super().__init__(db, Store, history, cache)
...
class InventoryRepository(BaseRepository):
    ...
    def create(self, data):
        ...
        if not self._record_exists("film", "film_id", film_id):
            raise ValueError(
                f"La pelicula con film_id={film_id} no existe. "
                "Seleccione uno de los IDs mostrados en el menu."
            )
        if not self._record_exists("store", "store_id", store_id):
            raise ValueError(
                f"La tienda con store_id={store_id} no existe. "
                "Seleccione uno de los IDs mostrados en el menu."
            )

        return super().create(data)

    def _record_exists(self, table, primary_key, entity_id):
        if entity_id is None:
            return False
        with self.db.cursor() as cursor:
            cursor.execute(
                f"SELECT 1 FROM {table} WHERE {primary_key} = %s LIMIT 1",
                (entity_id,),
            )
